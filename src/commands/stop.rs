...
fn retain(command: &StopCommand, choice: GameType) -> bool {
    command.games.iter().any(|&game| game == choice)
}
        ...
        match game {
            GameType::Avalon => {
                ...
                }
                {
                    ...
                    this_cmd.edit_command(&state, guild, interaction.data.command).await?;
                }
                Ok(interaction)
            }
            GameType::Coup => todo!(),
