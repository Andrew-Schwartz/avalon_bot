...
use crate::http::model::{GuildMemberUserless, Id};
use crate::http::model::channel::ChannelType;
use crate::http::model::emoji::Emoji;
use crate::http::model::ids::*;
use crate::http::model::Png;
...
impl Color {
    pub const GOLD: Self = Color::from_rgb(0xBC, 0x9D, 0x46);

    pub const BLUE: Self = Color::from_rgb(0x36, 0x93, 0xD1);
    pub const RED: Self = Color::from_rgb(0xBA, 0x46, 0x50);
}
...
impl EmbedField {
    fn checked<S: ToString, V: ToString>(name: S, value: V, inline: bool) -> EmbedField {
        let name = name.to_string();
        assert!(!name.is_empty(), "field names cannot be empty");
        let value = value.to_string();
        assert!(!value.is_empty(), "field values cannot be empty");
